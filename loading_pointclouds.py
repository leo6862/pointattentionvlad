...
def load_processed_pc_file(filename):
    ...
    pc = np.reshape(np.array(pc),(pc.shape[0]//19, 19))
    return pc

def load_processed_pc_files(filenames):
    pcs = []
    for filename in filenames:
        # print(filename)
        pc = load_processed_pc_file(filename)
...
def get_query_tuple_nn_neighbor(dict_value, num_pos, num_neg, QUERY_DICT, hard_neg=[], other_neg=False):
        # get query tuple for dictionary entry
        # return list [query,positives,negatives]

    query = load_processed_pc_file(dict_value["query"])  # Nx19
    ...
    for i in range(num_pos):
        pos_files.append(QUERY_DICT[dict_value["positives"][i]]["query"])
    #positives= load_pc_files(dict_value["positives"][0:num_pos])
    positives = load_processed_pc_files(pos_files)
    ...
    negatives = load_processed_pc_files(neg_files)
    ...
    # For Quadruplet Loss
        ...
        neg2 = load_processed_pc_file(QUERY_DICT[possible_negs[0]]["query"])
        return [query, positives, negatives, neg2]
