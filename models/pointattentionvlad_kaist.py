...
def group_points(points, idx):
    ...
    # print('idx[..., None].shape = {}'.format(idx[..., None].shape)) # 2 12 1 
    #2 12 3  \\\\\ B  S*K C
    # print('idx[..., None].expand(-1, -1, points.size(-1)).shape = {}'.format(idx[..., None].expand(-1, -1, points.size(-1)).shape))
...
class NetVLADLoupe(nn.Module):
    ...
    def forward(self, x):
        # print('netvlaad input x size = {}'.format(x.shape))
...
class PointAttentionVLAD_kaist(nn.Module):
    def __init__(self,num_points=4096,output_dim=256,layer_number=3):
        super(PointAttentionVLAD_kaist,self).__init__()
        inchannel = 3
        hidden_channel_1 = 64
        ...
        self.net_vlad = NetVLADLoupe(feature_size=hidden_channel_3, max_samples=3000, cluster_size=64,
    ...
    def forward(self,x):
        ...
        fps_idx = farthest_point_sample(x_raw, 3000) #B 2000
        ...
        global_attention_grouped_x = torch.cat((global_attention_grouped_x.permute(1,2,0),grouped_x.permute(0,2,1)),dim=1)
...
if __name__ == "__main__":
    # model = Sample_and_Grouping()
    # data = torch.randn((5,4096,3))
    # out = model(data)
    # print('out.shape = {}'.format(out.shape))
    # from torchvision.models import resnet50
    # from thop import profile
    import thop
    model = PointAttentionVLAD_kaist().cpu()
