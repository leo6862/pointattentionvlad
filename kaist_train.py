...
# modified sample point num in FPS and use larger submap 
import models.PointAttentionVLAD_kaist as PointAttentionVLAD_kaist
...
parser.add_argument('--dataset_folder', default='/home/yyj/kaist_v2/',
...
# kaist_yyj/scripts/generating_training_tuple_v2.py
#TODO modify this
cfg.TRAIN_FILE = '/home/yyj/dl_dataset/kaist_v2/training_tuple.pickle'
cfg.TEST_FILE  = '/home/yyj/dl_dataset/kaist_v2/testing_tuple.pickle'
...
def train():
    ...
    model = PointAttentionVLAD_kaist.PointAttentionVLAD_kaist()
    ...
    for i in range(len(train_file_idxs)//cfg.BATCH_NUM_QUERIES):
        ...
        for j in range(cfg.BATCH_NUM_QUERIES):
            ...
            if (len(TRAINING_LATENT_VECTORS) == 0):
                q_tuples.append(
                    get_query_tuple_nn_neighbor(TRAINING_QUERIES[batch_keys[j]], cfg.TRAIN_POSITIVES_PER_QUERY, cfg.TRAIN_NEGATIVES_PER_QUERY,
...
def get_feature_representation(filename, model):
    model.eval()
    queries = load_processed_pc_files([filename])
    queries = np.expand_dims(queries, axis=1)
...
def get_latent_vectors(model, dict_to_process):
    ...
    for q_index in range((len(train_file_idxs) // batch_num * batch_num), len(dict_to_process.keys())):
        ...
        queries = load_processed_pc_files([dict_to_process[index]["query"]])
        queries = np.expand_dims(queries, axis=1)
...
def run_model(model, queries, positives, negatives, other_neg, require_grad=True):
    ...
    feed_tensor = feed_tensor.view((-1, 1, cfg.NUM_POINTS, 19))
